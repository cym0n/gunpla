...
$world->armies->[2]->position->x(300000);
...
is(@{$world->armies}, 5, "Scenario starts with 5 mechas");
...
my $commands = { 'Diver-1' => { command => 'rifle', params => 'MEC-Gelgoog'},
                 ...
                 'Zaku-1'    => { command => 'flymec', params => 'MEC-Diver-1' } };
is(Gunpla::Test::emulate_commands($world, $commands), 1, "Diver-2 shifted of few steps to test attack target on enemy death");
is(Gunpla::Test::emulate_commands($world, { 'Diver-2' => { command => 'rifle', params => 'MEC-Gelgoog'} }), 10, "Diver-1 shoot to a very weak Gelgoog with a rifle. Diver-2 is aiming the same target");
...
is_deeply($world->get_events('Zaku-1'), ["contact lost: Diver-1"], "Zaku-1 can't pursue Diver-1 anymore");
is($world->armies->[0]->waiting, 1, "Zaku-1 waiting after losing contact");
ok(! $world->sighting_matrix->see_faction('eagle', 'Diver-1'), "Eagle faction can't see Diver anymore");
is(@{$world->armies}, 4, "4 mechas left");
