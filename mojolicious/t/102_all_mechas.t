use Mojo::Base -strict;

use Test::More;
use Test::Mojo;

use lib 'lib';
use Data::Dumper;
use Gunpla::World;

diag("Drop gunpla_autotest db on local mongodb");
my $mongo = MongoDB->connect(); 
my $db = $mongo->get_database('gunpla_autotest');
$db->drop();


diag("Generate a world and save it on db");
my $world = Gunpla::World->new(name => 'autotest');
$world->init();
$world->save();

my $t = Test::Mojo->new('GunplaServer');

$t->get_ok('/game/mechas?game=autotest')->status_is(200)->json_is(
{ 'mechas' => [
  {
    'name' => 'Diver',
    'faction' => 'wolf',
    'position' => { 'z' => 0, 'y' => 0, 'x' => 500000 },
    'command' => '',
    '_id' => '5e826fa8af404c1b91b5ba68',
    'waiting' => 1,
    'course' => { 'axis' => '', 'steps' => 0, 'direction' => 0 },
    'destination' => { 'x' => 500000, 'y' => 0, 'z' => 0 }
  },
  {
    'name' => 'Zaku',
    'faction' => 'eagle',
    'position' => { 'x' => -500000, 'y' => 0, 'z' => 0 },
    'command' => '',
    '_id' => '5e826fa8af404c1b91b5ba69',
    'waiting' => 1,
    'course' => { 'axis' => '', 'steps' => 0, 'direction' => 0 },
    'destination' => { 'x' => -500000, 'z' => 0, 'y' => 0 },
  }]
});
open(my $log, "> /tmp/out1.log");
print {$log} Dumper($t->tx->res->json) . "\n";
close($log);

diag("Drop gunpla_autotest db on local mongodb for final cleanup");
$db = $mongo->get_database('gunpla_autotest');
$db->drop();


done_testing();
